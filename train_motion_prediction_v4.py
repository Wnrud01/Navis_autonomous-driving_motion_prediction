#!/usr/bin/env python3
"""Train Motion Prediction V4 (polyline map + lane-anchored modes) with aWTA."""
...
from src.train_motion_prediction_v4 import (
    POLY_K, MotionPredictorV4, WindowSampleCollateV4, load_compatible_state,
)
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v4_polyline")
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V4 — POLYLINE MAP + LANE-ANCHORED MODES")
    ...
    train_loader = make_loader(
        ...
        WindowSampleCollateV4(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, True), pin,
    )
    val_loader = make_loader(
        val_ds, args.batch_scenes, args.workers, args.prefetch, False,
        WindowSampleCollateV4(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, False), pin,
    )
    print(
    ...
    model = MotionPredictorV4(hidden=args.hidden, modes=6).to(device)
    ...
    print(
        f"\nV4 Training Completed! Best Error Score: {best_error_score:.4f}, "
