"""Adaptive Winner-Takes-All Loss V17 with Deep Supervision for X-Large Two-Stage Decoders.

Features:
1. Stage 1 Proposal aWTA Loss: 3-Stage Annealing (Top-4 -> Top-2 -> Top-1, tau 1.5 -> 0.1).
2. Stage 2 Refinement aWTA Loss: Supervises locally refined trajectories and goals.
3. Focal Multi-Future Classification + Hard Winner CE on Stage 2 logits.
...
class AdaptiveWTALossV17(nn.Module):
    def __init__(
        self,
        modes: int = 6,
        tau_start: float = 1.5,
    ...
    def forward(
        ...
        metrics = {
            ...
            "minade6_batch": ade_modes_2.min(dim=-1).values.mean().detach(),
            "minade1_batch": ade_modes_2[b_idx, top1_mode].mean().detach(),
        }
