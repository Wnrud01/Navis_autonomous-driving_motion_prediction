#!/usr/bin/env python3
"""Train V8 (V6 + lane-signal mix + kinematics) from V6 weights."""
...
from src.train_motion_prediction_v8 import POLY_K, MotionPredictorV8, WindowSampleCollateV8
...
# Reuse V3 loops but with our model_forward by monkeypatching the imported name.
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v8_interact")
    parser.add_argument("--resume-ckpt", default=r"E:\motion_prediction\checkpoints\v6_temporal\best_error_score.pth")
    ...
    parser.add_argument("--weight-fde", type=float, default=0.25)
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V8 — LANE+SIGNAL MIX + KINEMATICS ON V6")
    print(f" Data Root:       {args.data_root}")
    print(f" Output Dir:      {args.out_dir}")
    print(f" Resume Checkpoint: {args.resume_ckpt}")
    print(f" Epochs:          {args.epochs} (Batch: {args.batch_scenes}, LR: {args.lr})")
    print(f" Mix tokens:      {args.poly_k} lane-signal  | kin 8-d | neighbors {args.neighbor_k}")
    print(f" Loss extras:     time_weight_end={args.time_weight_end}, weight_fde={args.weight_fde}")
    print(f" AMP:             {args.amp}  Device: {device}")
    print("=" * 80, flush=True)
    ...
    train_loader = make_loader(
        ...
        WindowSampleCollateV8(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, args.poly_k, True), pin,
    )
    val_loader = make_loader(
        val_ds, args.batch_scenes, args.workers, args.prefetch, False,
        WindowSampleCollateV8(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, args.poly_k, False), pin,
    )
    print(
    ...
    model = MotionPredictorV8(hidden=args.hidden, modes=6).to(device)
    ...
    if args.resume_ckpt and os.path.exists(args.resume_ckpt):
        ...
        print(f"-> Loaded {loaded}/{len(model.state_dict())} tensors", flush=True)
        if missing:
            print(f"-> New keys: {missing}", flush=True)
    ...
    with open(os.path.join(args.out_dir, "config.json"), "w", encoding="utf-8") as f:
        json.dump({**vars(args), "device": str(device), "params": n_params}, f, ensure_ascii=False, indent=2)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        log_msg = (
            ...
            f"Train minADE6 {train_res['train_minade6']:.3f} minADE1 {train_res['train_minade1']:.3f} | "
            f"Val minADE6 {val_res['val_minade6']:.4f} minADE1 {val_res['val_minade1']:.4f} "
            f"minFDE6 {val_res['val_minfde6']:.4f} Error {val_res['val_error_score']:.4f}"
        )
        print(log_msg, flush=True)
        with open(log_file, "a", encoding="utf-8") as f:
            f.write(log_msg + "\n")
        torch.save({"epoch": epoch, "model_state": model.state_dict(), "metrics": epoch_record}, os.path.join(args.out_dir, "last.pth"))
        if val_res["val_error_score"] < best_error_score:
            best_error_score = val_res["val_error_score"]
            torch.save({"epoch": epoch, "model_state": model.state_dict(), "metrics": epoch_record}, os.path.join(args.out_dir, "best_error_score.pth"))
            print(f"[NEW BEST Error Score]: {best_error_score:.4f}", flush=True)
        if val_res["val_minade6"] < best_minade6:
            best_minade6 = val_res["val_minade6"]
            torch.save({"epoch": epoch, "model_state": model.state_dict(), "metrics": epoch_record}, os.path.join(args.out_dir, "best_minade6.pth"))
            print(f"[NEW BEST minADE6]: {best_minade6:.4f}m", flush=True)
        if args.max_train_steps:
            break

    print(f"\nV8 done. Best Error {best_error_score:.4f}  Best minADE6 {best_minade6:.4f}m", flush=True)
