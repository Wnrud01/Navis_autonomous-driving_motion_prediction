#!/usr/bin/env python3
"""Train V11 for minADE6: no 8s-goal warp, uniform ADE loss, keep 2 modes in aWTA."""
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v11_ade6")
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V11 — MINADE6 FIRST (no 8s goal warp)")
    print(" Decoder: cumsum + refine only. Loss: uniform ADE, goal aux, no FDE term.")
    print(" aWTA keeps Top-2 at the end so 6 modes stay diverse.")
    ...
    model = MotionPredictorV11(hidden=args.hidden, modes=6).to(device)
    ...
    if args.resume_ckpt and os.path.exists(args.resume_ckpt):
        ...
        print("-> random init: V11 ADE6-first (no V10 weight load, decoder geometry changed)", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.max_train_steps:
            break
    print(f"V11 done. Best minADE6 {best_minade6:.4f} Error {best_error_score:.4f}", flush=True)
