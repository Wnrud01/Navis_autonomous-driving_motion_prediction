#!/usr/bin/env python3
"""High-Fidelity Visualizer for Motion Prediction V16 Two-Stage Model (All-Time Best).
...
from src.train_motion_prediction_v1 import (
    TYPE_TO_INDEX, local_xy, local_vec,
)
...
def predict_scene_v16(model, static: dict, window: dict, device: torch.device):
...
def render_scene(pack_path: str, model, device: torch.device, out_dir: str, prefix: str):
    ...
    preds = predict_scene_v16(model, static, window, device)
    if preds is None:
        return
    ...
    title_txt = f"V16 Two-Stage Refinement Model | Multi-Agent Prediction (Val Scene: {prefix})"
    ...
    frames = []
    step_indices = list(range(0, 80, 5))  # 16 frames
    for s_idx in step_indices:
        ...
        for i, r in enumerate(rows):
            ...
            # Current vehicle position at step s_idx
        ...
        ax_f.text(0.5, 0.96, f"V16 Rollout | t = +{t_sec:.1f}s / 8.0s", transform=ax_f.transAxes, color="#f8fafc", fontsize=12, fontweight="bold", ha="center")
        fig_f.canvas.draw()
...
def main():
    ...
    print("=" * 80)
    print(" RENDERING HIGH-FIDELITY V16 SHOWCASE VISUALIZATIONS & GIFS")
    print("=" * 80)

    model = MotionPredictorV16(hidden=256, modes=6).to(device)
    ckpt_path = r"E:\motion_prediction\checkpoints\v16_twostage\best_error_score.pth"
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    load_compatible_state(model, ckpt["model_state"])
    model.eval()
    print(f"-> Loaded V16 Best Checkpoint (Val Error: {ckpt['metrics']['val_error_score']:.4f})", flush=True)
    ...
    for idx, sp in enumerate(sample_scenes, start=1):
        ...
        # Overwrite the primary root showcase files with Scene 1 & 2
        if idx == 1:
            im = Image.open(png_p)
            ...
            print("-> Updated root showcase GIF and PNG with V16 Best model predictions!")

    # 3. Create 2x5 Multi-Scene Collage PNG
    ...
    print("ALL VISUALIZATIONS AND GIFS SUCCESSFULLY GENERATED!")
