#!/usr/bin/env python3
"""Train Motion Prediction V15: All-Mode Training + Lane-Following Loss.

Key differences from V13:
1. aWTA keeps Top-6 throughout (all modes always receive gradient).
2. Lane-following auxiliary loss forces all 6 trajectories onto valid roads.
3. Reduced diversity loss to allow natural convergence on unambiguous scenes.
4. Warm-starts from V13 best checkpoint (pretrained generator).
"""
...
from src.losses.awta_loss_v15 import AdaptiveWTALossV15
...
def evaluate_validation_v15(model, val_loader, criterion, device, epoch, total_epochs, amp_dtype, max_steps=None):
    model.eval()
    ...
    ade_all_sum = torch.zeros((), device=device)
    ade_worst_sum = torch.zeros((), device=device)
        ...
        for step, samples in enumerate(val_loader, start=1):
            ...
            with torch.amp.autocast("cuda", dtype=amp_dtype, enabled=(device.type == "cuda")):
                ...
                loss, metrics = criterion(
                    ...
                    map_feat=samples["map_feat"], map_valid=samples["map_valid"],
            ...
            ade_all_sum += ade_modes.mean(dim=-1).sum()
            ade_worst_sum += ade_modes.max(dim=-1).values.sum()
            pick_correct_sum += (top1_mode == best_mode_6).float().sum()
    ...
    if n_targets == 0:
        return {k: 0.0 for k in [
            "val_loss", "val_minade6", "val_minade1", "val_minfde6", "val_minfde1",
            "val_error_score", "val_pick_acc", "val_ade_all_mean", "val_ade_worst",
        ]}
    ...
    return {
        ...
        "val_ade_all_mean": float(ade_all_sum / n_targets),
        "val_ade_worst": float(ade_worst_sum / n_targets),
    }


def train_one_epoch_v15(model, train_loader, optimizer, scaler, criterion, device, args, epoch, amp_dtype, use_scaler):
    model.train()
    ...
    lane_sum = torch.zeros((), device=device)
    n_targets = 0
    n_batches = 0
    cur_tau = 1.0
    ...
    for step, samples in enumerate(train_loader, start=1):
        ...
        ade_all_sum += metrics["ade_all_mean"] * b
        ade_worst_sum += metrics["ade_worst"] * b
        pick_sum += metrics["pick_acc"] * b
        lane_sum += metrics["loss_lane"] * b
        n_targets += b
        n_batches += 1
        cur_tau = metrics["tau"]
        ...
        if step % args.log_every == 0 or step == len(train_loader):
            ...
            print(
                ...
                f"ade_all {metrics['ade_all_mean']:.4f} ade_worst {metrics['ade_worst']:.4f} "
                f"lane {metrics['loss_lane']:.4f} pick {metrics['pick_acc']:.3f} tau {cur_tau:.3f} lr {lr:.2e}",
                flush=True,
            )
    ...
    return {
        ...
        "train_ade_all_mean": float(ade_all_sum / max(n_targets, 1)),
        "train_ade_worst": float(ade_worst_sum / max(n_targets, 1)),
        "train_pick_acc": float(pick_sum / max(n_targets, 1)),
        "train_lane_loss": float(lane_sum / max(n_targets, 1)),
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v15_allmode")
    ...
    parser.add_argument("--warmstart-ckpt", default=r"E:\motion_prediction\checkpoints\v13_ade6\best_minade6.pth",
                        help="Load V13 weights but start from epoch 1")
    ...
    parser.add_argument("--weight-div", type=float, default=0.02)
    parser.add_argument("--weight-lane", type=float, default=0.3)
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V15 — ALL-MODE TRAINING + LANE-FOLLOWING LOSS")
    print(" Key: Top-6 WTA (all modes always trained) + Lane snapping loss")
    print(f" Warm-start: {args.warmstart_ckpt or '(none)'}")
    print(f" Epochs {args.epochs}  batch {args.batch_scenes}  lr {args.lr}  AMP {args.amp}")
    print(f" top_m={args.top_m} (fixed)  tau {args.tau_start}→{args.tau_end}  w_lane={args.weight_lane}  w_div={args.weight_div}")
    print("=" * 80, flush=True)
    ...
    # Use V13 model architecture (unchanged)
    model = MotionPredictorV13(hidden=args.hidden, modes=6).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"-> V15 Model (= V13 architecture): {n_params/1e6:.3f}M parameters", flush=True)

    criterion = AdaptiveWTALossV15(
        modes=6,
        tau_start=args.tau_start, tau_end=args.tau_end, tau_cls=args.tau_cls,
        top_m=args.top_m,
        weight_reg=args.weight_reg, weight_goal=args.weight_goal,
        weight_cls=args.weight_cls, weight_hard_cls=args.weight_hard_cls,
        weight_div=args.weight_div, weight_lane=args.weight_lane,
        horizon_weight_scale=args.horizon_weight,
    ).to(device)
    ...
    if args.resume_ckpt and os.path.exists(args.resume_ckpt):
        ...
        missing, _ = load_compatible_state(model, ckpt["model_state"])
        loaded = sum(1 for k in model.state_dict() if k not in missing)
        print(f"-> WARM-START from V13: loaded {loaded}/{len(model.state_dict())} keys, epoch 1", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(
            ...
            f"ADE_all {train_res['train_ade_all_mean']:.3f} worst {train_res['train_ade_worst']:.3f} "
            f"lane {train_res['train_lane_loss']:.4f} pick {train_res['train_pick_acc']:.3f} | "
            f"Val ADE6 {val_res['val_minade6']:.4f} ADE1 {val_res['val_minade1']:.4f} "
            f"ADE_all {val_res['val_ade_all_mean']:.4f} worst {val_res['val_ade_worst']:.4f} "
