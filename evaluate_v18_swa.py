#!/usr/bin/env python3
"""Evaluate SWA & Soft Density NMS on Motion Prediction V18 Fine-Tuned Model.

Averages checkpoints from checkpoints/v18_speed_finetune and runs NMS sweep with sigma=[1.5, 2.0, 2.5, 3.0, 3.5]
...
def average_checkpoints_v18(ckpt_paths: list[str]) -> dict:
    print(f"-> Averaging {len(ckpt_paths)} V18 checkpoints:")
    for p in ckpt_paths:
        print(f"   * {p}")
...
def main():
    ...
    parser.add_argument("--ckpt-dir", default=r"E:\motion_prediction\checkpoints\v18_speed_finetune")
    ...
    print("=" * 80)
    print(" SWA CHECKPOINT AVERAGING + SOFT DENSITY NMS EVALUATION (V18 FINE-TUNED)")
    print("=" * 80)

    avg_state = average_checkpoints_v18(ckpt_paths)
    ...
    evaluate_nms_v17(model, val_loader, device, amp_dtype, sigma_list=[1.5, 2.0, 2.5, 3.0, 3.5])
