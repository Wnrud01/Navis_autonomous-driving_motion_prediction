#!/usr/bin/env python3
"""Train Motion Prediction V7 (V3 free-goal + scene attention + all targets)."""
...
def evaluate_validation_v7(model, val_loader, criterion, device, epoch, total_epochs, amp_dtype, max_steps=None):
    model.eval()
...
def train_one_epoch_v7(model, train_loader, optimizer, scaler, criterion, device, args, epoch, amp_dtype, use_scaler):
    model.train()
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v7_scene_v3")
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V7 — V3 FREE-GOAL + SCENE ATTENTION + ALL TARGETS")
    ...
    train_loader = make_loader(
        ...
        WindowSampleCollateV7(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, True), pin,
    )
    val_loader = make_loader(
        val_ds, args.batch_scenes, args.workers, args.prefetch, False,
        WindowSampleCollateV7(args.max_targets, args.neighbor_k, args.signal_k, args.map_k, False), pin,
    )
    print(
    ...
    model = MotionPredictorV7(hidden=args.hidden, modes=6).to(device)
    ...
    print(
        f"\nV7 Training Completed! Best Error Score: {best_error_score:.4f}, "
