#!/usr/bin/env python3
"""Train Motion Prediction V14: Trajectory-Aware Dense Scorer + ADE-Proportional Margin Ranking Loss."""
...
from src.train_motion_prediction_v14 import MotionPredictorV14
from src.losses.awta_loss_v14 import AdaptiveWTALossV14
...
def evaluate_validation_v14(model, val_loader, criterion, device, epoch, total_epochs, amp_dtype, max_steps=None):
    model.eval()
...
def train_one_epoch_v14(model, train_loader, optimizer, scaler, criterion, device, args, epoch, amp_dtype, use_scaler):
    model.train()
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v14_fixed")
    ...
    parser.add_argument("--warmstart-ckpt", default=r"E:\motion_prediction\checkpoints\v13_ade6\best_minade6.pth",
                        help="Load weights from this checkpoint but start training from epoch 1 (warm start, not resume)")
    ...
    parser.add_argument("--weight-cls", type=float, default=0.5)
    parser.add_argument("--weight-rank", type=float, default=0.3)
    parser.add_argument("--weight-div", type=float, default=0.08)
    parser.add_argument("--horizon-weight", type=float, default=0.5)
    parser.add_argument("--rank-margin-base", type=float, default=0.2)
    parser.add_argument("--rank-margin-alpha", type=float, default=0.5)
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V14 — TRAJECTORY-AWARE SCORER + MARGIN RANKING LOSS")
    print(" Innovations: Trajectory-Map Dense Cross-Attn Scorer + ADE-Proportional Margin Loss")
    ...
    model = MotionPredictorV14(hidden=args.hidden, modes=6).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"-> V14 Model parameters: {n_params/1e6:.3f}M", flush=True)

    criterion = AdaptiveWTALossV14(
        modes=6,
        ...
        weight_cls=args.weight_cls, weight_rank=args.weight_rank,
        weight_div=args.weight_div, horizon_weight_scale=args.horizon_weight,
        rank_margin_base=args.rank_margin_base, rank_margin_alpha=args.rank_margin_alpha,
    ).to(device)
    ...
    if args.resume_ckpt and os.path.exists(args.resume_ckpt):
        ...
        if os.path.isfile(metrics_file):
            ...
            if history:
                ...
                best_pick_acc = max(h.get("val_pick_acc", 0.0) for h in history)
    elif args.warmstart_ckpt and os.path.exists(args.warmstart_ckpt):
        # Warm-start: load V13 generator weights but start from epoch 1 with fresh optimizer
        ...
        print(f"-> WARM-START from V13: loaded {loaded}/{len(model.state_dict())} keys "
              f"({new_keys} new scorer keys randomly initialized), epoch 1", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(
            ...
            f"Train ADE6 {train_res['train_minade6']:.3f} ADE1 {train_res['train_minade1']:.3f} pick {train_res['train_pick_acc']:.3f} | "
