#!/usr/bin/env python3
"""Train Motion Prediction V17: X-Large 58M Architecture with Polyline & Social Graph Attention."""
...
from src.losses.awta_loss_v17 import AdaptiveWTALossV17
...
def model_forward_v17(model, samples):
    return model(
        samples["target_hist"],
        samples["neighbors"],
...
def evaluate_validation_v17(model, val_loader, criterion, device, epoch, total_epochs, amp_dtype, max_steps=None):
    model.eval()
...
def train_one_epoch_v17(model, train_loader, optimizer, scaler, criterion, device, args, epoch, amp_dtype, use_scaler):
    model.train()
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v17_xlarge")
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V17 — X-LARGE 58M TWO-STAGE GRAPH TRANSFORMER")
    print(" Hidden=768  Heads=12  Polyline Graph Attn  Social Graph Attn  Two-Stage Refinement")
    print(f" Epochs {args.epochs}  batch {args.batch_scenes}  lr {args.lr}  AMP {args.amp}  dropout {args.dropout}")
    print("=" * 80, flush=True)
    ...
    print(f"-> V17 X-Large Model parameters: {n_params/1e6:.3f}M", flush=True)

    criterion = AdaptiveWTALossV17(
        modes=6,
