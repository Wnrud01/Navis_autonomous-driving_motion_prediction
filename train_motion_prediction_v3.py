#!/usr/bin/env python3
"""Train Motion Prediction V3 (map + neighbor history + lead/follow attention) with aWTA."""
...
from src.train_motion_prediction_v3 import (
    MAP_K, MotionPredictorV3, WindowSampleCollateV3, load_compatible_state,
)
...
def train_one_epoch_v3(model, train_loader, optimizer, scaler, criterion, device, args, epoch, amp_dtype, use_scaler):
    ...
    for step, samples in enumerate(train_loader, start=1):
        ...
        if not torch.isfinite(loss):
            print(
                f" skip batch step {step} n={int(samples['target_hist'].shape[0])}: "
                f"loss is NaN/Inf (forward exploded, not a data filter)",
                flush=True,
            )
            continue
        if use_scaler:
            ...
            if not torch.isfinite(grad_norm):
                optimizer.zero_grad(set_to_none=True)
                print(f" skip batch step {step}: grad is NaN/Inf, not stepping", flush=True)
                continue
...
def main():
    ...
    parser.add_argument("--out-dir", default=r"E:\motion_prediction\checkpoints\v3_map_attn")
    parser.add_argument("--resume-ckpt", default=r"E:\motion_prediction\checkpoints\v2_awta_lane\best_error_score.pth")
    ...
    print("=" * 80)
    print(" MOTION PREDICTION V3 — MAP + NEIGHBOR HISTORY + LEAD/FOLLOW ATTN")
    ...
    model = MotionPredictorV3(hidden=args.hidden, modes=6).to(device)
    ...
    print(
        f"\nV3 Training Completed! Best Error Score: {best_error_score:.4f}, "
